...
def get_tasks(user: user_schema.User, is_done: bool = None):
    if is_done is None:
        task_by_user = TodoModel.filter(TodoModel.user_id == user.id).order_by(
            TodoModel.created_at.desc()
        )
    else:
        task_by_user = TodoModel.filter(
...
def get_task(task_id: int, user: user_schema.User):
    task = TodoModel.filter(
        (TodoModel.id == task.id) & (TodoModel.user_id == user.id)
    ).first()
    ...
    return todo_schema.Todo(
        id=task.id, title=task.title, is_done=task.is_done, created_at=task.created_at
    )
